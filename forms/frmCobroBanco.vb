...
finalizar:
    ...
    Private Sub GrabarPago()
        ...
        Else
            Try
                comando.CommandText = "SELECT * FROM comprobte WHERE TipoCpbte = 'MOV'"
                dt = New DataTable
                da = New MySqlDataAdapter(comando)
                da.Fill(dt)
                ...
                If dt.Rows.Count > 0 Then
                    id = CStr(row("NroCpbte"))
                    id = id + 1
                End If

                comando = New MySqlCommand("INSERT INTO ctactesoc VALUES(@id, @matricula, @fechapago, @detalle, @debe, @haber, @saldo, @periodo, @estado, @pagado, @obs)", conexion)
                ...
                comando.Parameters.AddWithValue("@periodo", periodo)
                comando.Parameters.AddWithValue("@estado", "PAGO BANCO")
                comando.Parameters.AddWithValue("@pagado", fechapago)
                comando.Parameters.AddWithValue("@obs", obs)
                comando.ExecuteNonQuery()

                comando = New MySqlCommand(("UPDATE comprobte SET NroCpbte = '" & id & "' WHERE TipoCpbte = 'MOV'"), conexion)
                comando.ExecuteNonQuery()
